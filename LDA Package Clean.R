# Runs Linear Discriminant Analysis on data set
# Omits columns 21, 22, and 29 (Soil7, Soil8, and Soil15) due to sparseness
# Output: matrixLDA - List of misclassification rates from the myN number of run trials
# User-controlled variables: myN (number of trials)

# Define global variables
myN=1 # Number of trials
matrixLDA <- matrix(rep(0,myN),ncol=1) # Matrix of misclassification rates
...
for (a in 1:myN) {
  ...
  testing_X <- testing_X[1:11340,] # The test set is limited to 11,340 observations
  testing_Y <- testing_Y[1:11340]
  
  # Model Building
  model <- lda(training_Y~.,training_X)
  pred.class=predict(model,testing_X)$class
  
  # Misclassification rate
  matrixLDA[a] <- 100*sum(pred.class != testing_Y)/length(pred.class)
}

# Display the misclassification error
print(sprintf("misclassification rate %f percent", mean(matrixLDA)))
print(sprintf("standard deviation %f", sd(matrixLDA)))

# Confusion matrix
table(testing_Y, pred.class)